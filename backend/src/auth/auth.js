...
passport.use(new GoogleStrategy({
  ...
  function (request, accessToken, refreshToken, profile, done) {
    ...
    User.findOrCreate({ where: { email: profile.email } }).then(([user, created]) => {
      const body = {
        id: user.id,
        email: user.email,
...
passport.use(new FacebookStrategy({
  ...
  clientSecret: config.facebook.secrete,
  callbackURL: config.apiUrl + "/user/signin/facebook/callback",
},
